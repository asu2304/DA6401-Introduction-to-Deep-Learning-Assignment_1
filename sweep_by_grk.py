...
def start_train_with_sweep():
    wandb.init()
    config = wandb.config
    sweep_name = f"hl_{config.num_layers}_bs_{config.batch_size}_ac_{config.activation}"
...
if __name__ == "__main__":
    ...
    # Create sweep and explicitly print ID and URL
    sweep_id = wandb.sweep(sweep_configurations, project="sweep_project_question_4")
    entity = wandb.api.viewer().get('entity', config['wandb_entity']) or config['wandb_entity']
    print(f"Create sweep with ID: {sweep_id}")
    print(f"Sweep URL: https://wandb.ai/{entity}/sweep_project_question_4/sweeps/{sweep_id}")

    # Start the agent
    wandb.agent(sweep_id, function=start_train_with_sweep)